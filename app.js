/* app.js — PeerJS connection (QR handshake), chess.js game logic,
   stress-test timers, telemetry dashboard and Chart.js graph. */

import { Chess } from '[cdn.jsdelivr.net](https://cdn.jsdelivr.net/npm/chess.js@1.4.0/+esm)';
import { Chessground } from '[cdn.jsdelivr.net](https://cdn.jsdelivr.net/npm/chessground@9.1.1/+esm)';
...
if (!crossOriginIsolated && 'serviceWorker' in navigator && window.isSecureContext) {
  navigator.serviceWorker.register('./sw.js').then(() => {
    // First load can never be controlled by a brand-new SW: reload exactly once.
...
let peerInfo = null;             // { cores, isolated }
let mode = null;                 // 'match' | 'stress'
let cfg = null;                  // { threads, depth, duration }
let chess = null, ground = null;
let myColor = null, thinking = false, tStart = 0;
let lastTel = { nps: 0, nodes: 0, depth: 0 };
let ttds = [];
let stressTimer = null, stressEndTimer = null, samples = [];
...
function engineSources() {
  ...
  if (crossOriginIsolated) {
    s.push({ label: 'local multi-threaded',  js: local('stockfish-18-lite.js'), threaded: true });
    s.push({ label: 'CDN multi-threaded',    js: SF_CDN + 'stockfish-18-lite.js', threaded: true, crossOrigin: true });
  }
  s.push({ label: 'local single-threaded', js: local('stockfish-18-lite-single.js') });
  s.push({ label: 'CDN single-threaded',   js: SF_CDN + 'stockfish-18-lite-single.js', crossOrigin: true });
  return s;
}
  ...
  switch (m.type) {
    case 'loaded':
      ...
      $('engine-status').textContent =
        `Engine: Stockfish 18 (${m.threaded ? 'multi-threaded' : 'single-threaded'})` +
        (crossOriginIsolated ? '' : ' — no cross-origin isolation, threads limited to 1');
      break;
    ...
    case 'fatal':
      $('engine-status').textContent = 'Engine failed to load. Self-host the stockfish/ files (see README).';
      break;
  }
}
...
function startHost() {
  ...
  peer.on('open', (id) => {
    const qr = qrcode(0, 'M');
    qr.addData(QR_PREFIX + id);
    qr.make();
    $('qr-img').src = qr.createDataURL(6, 8);   // data: URL — no innerHTML
...
function startJoin() {
  ...
  $('btn-manual').addEventListener('click', () => {
    handleToken($('manual-token').value.trim());
  });
}
...
function validateMsg(raw) {
  ...
  if (++msgWindow.n > MAX_MSGS_SEC) return null;                    // rate limit
...
function handlePeerMsg(m) {
  switch (m.t) {
    ...
    case 'gameover': /* both sides detect it independently; informational only */ break;
...
async function startScanner() {
  try {
    ...
    $('scan-hint').textContent = 'Camera unavailable — use the manual token field below.';
    return;
  }
  const video = $('cam');
...
function beginMatch() {
  ...
  setStatus(`Match started — fixed depth ${cfg.depth}, ${cfg.threads} thread(s) each. You are ${myColor === 'w' ? 'White' : 'Black'}.`);
...
function onPeerMove(m) {
  if (mode !== 'match' || chess.turn() === myColor) { endRun('Protocol violation from peer.'); return; }
  if (!applyUci(m.uci)) {
    // Illegal move ⇒ desync or tampering. Never apply; stop the session.
...
function beginStress() {
  ...
  setStatus(`Stress test — go infinite for ${cfg.duration}s at ${cfg.threads} thread(s). Watch the throttling curve.`);
...
function initChart() {
  ...
  chart = new Chart($('chart'), {
    ...
    options: {
      ...
      scales: {
        x: { type: 'linear', title: { display: true, text: 'seconds' }, ticks: { color: '#94a3b8' } },
        y: { beginAtZero: true, title: { display: true, text: 'nodes / second' }, ticks: { color: '#94a3b8' } },
      },
...
function addPoint(ds, x, y) {
  ...
  data.push({ x, y });
  if (data.length > MAX_POINTS) data.shift();   // hard cap — no unbounded growth
  chart.update('none');
}
...
if (!window.isSecureContext) {
  setStatus('This app needs HTTPS (or localhost) — WebRTC, camera and WASM threads require a secure context.');
} else {
  setStatus(crossOriginIsolated
    ? 'Cross-origin isolated — multi-threaded engine available.'
    : 'Not yet isolated — single-threaded fallback (reloading once may fix this).');
  startWorker();
    }
